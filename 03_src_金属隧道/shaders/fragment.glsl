...
vec2 z,v,e=vec2(.000035,-.000035);
float t,tt,b,g,g2,bb;
...
vec2 mp( vec3 p ){ 
  ...
  for(int i=0;i<4;i++){
    np4.xyz=abs(np4.xyz)-vec3(1,1.2,0);
    np4.xyz = 2.*clamp(np4.xyz, vec3(0.), vec3(2,0.,4.3+bb)) - np4.xyz;
    np4=np4*(1.3)/clamp(dot(np4.xyz,np4.xyz),0.1,.92);
  }  

  vec2 h,t=fb(abs(np4.xyz)-vec3(2,0,0),5.);
  t.x/=np4.w;
  ...
  h=fb(abs(np4.xyz)-vec3(0,4.5,0),7.);
  h.x=max(h.x,-bo(p,vec3(20,5,5)));
  h.x/=np4.w*1.5;

  t=t.x<h.x?t:h;

  h=vec2(bo(np4.xyz,vec3(0.0,b*20.,0.0)),6);
  h.x/=np4.w*1.5;
...
void main() {
  ...
  vec3 ro=mix(vec3(1),vec3(-0.5,1,-1),ceil(sin(tt*.5)))
      *vec3(10,2.8+0.75*smoothstep(-1.5,1.5,1.5*cos(tt+0.2)),cos(tt*0.3)*3.1);
  ...
  if(z.y>0.){
    ...
    co=mix(sp+al*(dif),fo,min(fr,.2));
    co=mix(fo,co,exp(-.0003*t*t*t));
  }

  gl_FragColor = vec4(pow(co+g*.2+g2,vec3(0.65)),1.0);
}